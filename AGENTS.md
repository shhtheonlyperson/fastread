...
- As of 2026-05-16, that canonical path may be a symlink to a shared uploader. The default allowlist includes the canonical `play-uploader@example.iam.gserviceaccount.com` and the legacy `legacy-play-uploader@example.iam.gserviceaccount.com`; the selected account must have Play Console access to JustRead. The preflight verifies live Android Publisher API edit access, so a stale permission grant fails before any upload.
