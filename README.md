...
| --- | --- | --- |
| `FastReadUITests` | `FastReadUITests` (or the bundled `FastRead` test action) | Single happy-path XCUITest: launches the app reset, picks `test.epub` from the system Files picker, asserts the reader opens on 房思琪, taps `SKIP →`, opens `CONTENTS`, jumps to a chapter, and confirms the playhead moves. |
